...
import os
from tkinter.constants import END, LEFT, S
...
file.add_command(label='Save_as',image=save_as_icons,compound=tk.LEFT,accelerator='ctrl+Alt+s',command=save_as)
...
edit.add_command(label='clear_all',image=clear_all_icon,compound=LEFT,accelerator='ctrl+Alt+x',command=lambda:text_editor.delete(1.0,tk.END))
...
view.add_checkbutton(label='Tool bar',onvalue=True,offvalue=0,image=tool_bar_icon,variable=show_toolbar,compound=tk.LEFT,command=hide_toolbar)
view.add_checkbutton(label='Status bar',onvalue=1,offvalue=False,image=status_bar_icon,variable=show_statusbar,compound=tk.LEFT,command=hide_statusbar)
